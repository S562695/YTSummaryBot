...
def main():
    ...
    st.title("YTSummaryBot by AIFalcons 🎥")
    st.markdown('<style>h1{color: orange; text-align: center;}</style>', unsafe_allow_html=True)
    st.subheader('Chatbot that summarizes the youtube video❤️')
    st.markdown('<style>h3{color: pink;  text-align: center;}</style>', unsafe_allow_html=True)

    # Expander for app details
    with st.expander("About the App"):
        st.write("This chatbot allows you to summarize the YouTube video, This enhances content accessibility, saves time, aids in information retrieval, supports learning and research, facilitates language acquisition, and assists content creation. To built this chatbot, we use Llama2, Whisper, Haystack and Streamlit.")
